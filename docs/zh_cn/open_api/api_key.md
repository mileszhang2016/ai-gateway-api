...
| 参数名 | 类型 | 参数含义 | 补充描述 |
| - | - | - | - |
| list | []object | API-Key列表 | 元素字段同创建接口返回，quota_plan中包含balance字段 |
...
### 返回数据(Data内容)
字段同创建接口返回，quota_plan中包含balance字段。

#### 成功返回参数示例
同创建接口返回示例，但quota_plan中包含balance字段。
...
**说明**
- 删除API-Key时，级联删除其专属的quota_plan、rate_limit_policy及底层资源（如果这些资源不被其他API-Key或Entity引用）
