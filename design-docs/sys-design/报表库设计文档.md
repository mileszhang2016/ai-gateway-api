...
> 本文档描述**报表库**（如 `bfe_report`）的表结构设计。报表库与控制面库（见 [数据库设计文档.md](./数据库设计文档.md)，33 张控制面表）**分离部署、独立授权**，由 log-reader `mod_log_mysql` 插件写入、报表查询 API（`/report/*`，见 [details/报表查询模块.md](./details/报表查询模块.md)）读取。模块接口定义见 [api-define/OpenAPI接口定义/report.md](../api-define/OpenAPI接口定义/report.md)。
...
- [数据库设计文档.md](./数据库设计文档.md) — 控制面库 33 张表（与本文档互补）
