...
`before` / `after` 由各域 `*ParamToMap` 从参数结构构造。marshal 模式（`json.Marshal`→`Unmarshal` 往返）统一经 `model/ioperlog/param_to_map.go` 的 `ParamToMap`：nil 值键（部分更新中省略的字段）不物化为 null，避免幻影 diff（issue #201，覆盖 api_key / provider / cluster）；显式零值（如 `enabled=false`）为非 nil，仍保留并计入 diff。其余域（entity、route_rules、quota、iauth、iprotocol、iroute_conf、rate_limit_policy）以手写 nil-guard 构造，语义等价。
