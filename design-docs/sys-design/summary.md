...
| 数据库设计文档 | [数据库设计文档.md](./数据库设计文档.md) | 描述 `ai-gateway-api` 当前实现中全部 25 张持久化表的字段、约束、索引、JSON 字段结构以及表间逻辑关系，覆盖基础配置、集群、路由、证书、API-Key、Entity、配额、限流、模型定价等模块。 |
...
| 配额余额同步机制 | [details/配额余额同步机制.md](./details/配额余额同步机制.md) | 描述 Redis 作为余额唯一真实来源的架构，包括 OpenAPI 查询余额直接读 Redis、`QuotaCache` 批量读取接口（`BatchGetRemaining` / `GetInt64Batch`）、`QuotaResetScheduler` 调度器、`BalanceSyncManager` 的过期重置、`QuotaPlanManager.ApplyQuotaPlanChange` 对 quota_plan 变更的差异化处理、自然周/月重置逻辑、Redis Key 生命周期以及 `last_reset_at` 上移至 `quota_plans` 表的设计；包含 RMB 配额定点精度设计与 `Clock` 时钟注入。 |
