...
`APIKeyManager.DeleteAPIKey` 会级联删除 `quota_plan`、`rate_limit_policy`、`route_rules`，最后删除 `api_key`。
...
`EntityManager.DeleteEntity` 会检查子 Entity 是否存在，然后级联删除 `quota_plan`、`rate_limit_policy`、`route_rules`，最后删除 `entity`。
