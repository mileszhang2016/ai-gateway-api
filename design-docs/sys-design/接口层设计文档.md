...
│ ├── ai_cache/ # AI 缓存规则（/ai-cache-rules，集合全量读写）
...
│ ├── operation_log/ # 操作日志查询（/operation-logs）
...
└── innerapi_v1/ # 数据面 Inner-API（BFE 配置拉取）
    ...
    ├── ai_cache/ # AI 缓存规则导出（/configs/ai-cache-rule）
    ├── ai_route/
...
func endpoints() []*xreq.Endpoint {
    return merge(
        ...
        ai_cache.Endpoints, // 新增：AI 缓存规则（/ai-cache-rules，集合全量读写）
    )
}
```
...
| GET | `/entities/{id}/quota-plan` | 查询 Entity 配额计划（`balance` 直接读取 Redis） | `FeatureEntity + ActionRead` |
...
> 说明：创建 API-Key 时支持传入 `key`（全局唯一，更新时忽略），长度限制为 1-128 字符，与数据库 `api_keys.api_key varchar(128)` 保持一致；`GET /api-keys` 与 `GET /api-keys/{id}` 返回的 `quota_plan` 均包含 `balance`，`balance` 由 Model 层实时读取 Redis 填充；无限配额返回 sentinel balance（`used=0`, `remaining=100000000`）。

| Method | Path | 用途 | Authorizer |
|--------|------|------|------------|
| POST | `/api-keys` | 创建 API-Key | `FeatureAPIKey + ActionCreate` |
| GET | `/api-keys` | API-Key 列表（分页，`quota_plan` 含 `balance`，直接读 Redis） | `FeatureAPIKey + ActionReadAll` |
| GET | `/api-keys/{id}` | API-Key 详情（`quota_plan` 含 `balance`，直接读 Redis） | `FeatureAPIKey + ActionRead` |
...
| GET | `/api-keys/{id}/quota-plan` | 查询配额计划（含 `balance`，直接读 Redis） | `FeatureAPIKey + ActionRead` |
...
> - `PATCH /clusters/{cluster_name}` 的请求体**不能包含 `name`**；名称由 URI 路径参数唯一指定，若请求体携带 `name` 返回 422。
...
#### 4.2.8 操作日志（operation_log）

代码位置：`endpoints/openapi_v1/operation_log/`

> 说明：该接口为只读接口，写入行为由各业务 Manager 在写操作成功或失败后自动触发。

| Method | Path | 用途 | Authorizer |
|--------|------|------|------------|
| GET | `/operation-logs` | 分页查询操作日志 | `FeatureOperationLog + ActionReadAll` |

#### 4.2.9 EPP 实例池（epp_pool）

代码位置：`endpoints/openapi_v1/epp_pool/`

> 说明：单例池（池名由配置项 `RunTime.DefaultEPPInstancePoolName` 提供，默认 `EPP.pool`），`GET` 详情 + `PATCH` 全量替换；实例列表是部署事实，由部署流程在扩缩容/换机后 reconcile `PATCH`（全量替换，天然幂等）。校验组名非空唯一、实例 id 池内全局唯一、`(host, port)` 组合唯一、组规模为每组 1~2 实例（1=仅主，2=主+备，拒绝空组与 3 个及以上实例的组）。`PATCH` 后触发分配悬空自动修复（同组重选 primary 或跨组重分配），由 `model/epp_pool` 分配器完成。
...
#### 4.2.11 AI 缓存规则（ai_cache）

代码位置：`endpoints/openapi_v1/ai_cache/`

> 说明：AI 缓存规则为全局集合资源（配合 BFE `mod_ai_cache`，一期仅 Redis 精确匹配），照 `global-route-rules` 先例仅提供集合级 GET/PUT 全量读写，无 `/{id}` 端点；规则按数组顺序匹配（first-match-wins），顺序即优先级；无 `enabled` 字段，PUT 提交的列表即生效集合；响应隐藏内部 `id`。

| Method | Path | 用途 | Authorizer |
|--------|------|------|------------|
| GET | `/ai-cache-rules` | 全量查询 AI 缓存规则集合（`id` 升序，即导出到 BFE 的顺序） | `FeatureAICache + ActionRead` |
| PUT | `/ai-cache-rules` | 全量更新 AI 缓存规则集合（单事务 delete-all + insert-all 整体替换） | `FeatureAICache + ActionUpdate` |
...
| GET | `/configs/ai-cache-rule` | 导出 AI 缓存规则配置（BFE `mod_ai_cache` 数据源，产物 `ai_cache.data`） | `FeatureAICache + ActionExport` |
