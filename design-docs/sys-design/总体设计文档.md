...
5. **Entity / RateLimit / Quota 相关存储与 Manager**：在 `model/entity`、`model/rate_limit_policy`、`model/quota` 中分别初始化 EntityType、Entity、RateLimitPolicy、QuotaPlan、RouteRules 及其 Manager。
6. **模块导出 Manager**：`APIKeyRuleManager`、`ModBodyProcessManager`、`AIRouteExporter`。
7. **API-Key Manager**：注入 Quota/RateLimit/Entity 适配器，实现 `model/api_key` 与配额/限流体系的打通。
8. **周期重置**：`BalanceSyncManager`、`QuotaResetScheduler`，并启动调度器；`QuotaResetScheduler` 注入 Redis 分布式锁，保证多实例下同一时刻只有一个实例执行重置；`BalanceSyncManager` 只执行 `ResetExpiredBalances`，不再执行 `SyncAllBalances`。
