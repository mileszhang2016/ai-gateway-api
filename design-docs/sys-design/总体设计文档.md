...
| `model/quota` | QuotaPlan、BalanceSync、QuotaResetScheduler；`QuotaPlan` 支持 `unit=RMB`，`quota` 扩展为 `float64` / `DECIMAL(18,8)`；`last_reset_at` 保存在 `quota_plans` 表；`quota_balances` 表已删除，OpenAPI 余额直接读 Redis |
...
8. **周期重置**：`BalanceSyncManager`、`QuotaResetScheduler`，并启动调度器；`BalanceSyncManager` 只执行 `ResetExpiredBalances`，不再执行 `SyncAllBalances`。
