...
- **配额计划管理**：查询和重置配额余额；`QuotaPlan` 支持 `unit = "total_token"` 与 `unit = "RMB"`。
- **限流策略管理**：TPM、RPM、并发限制配置。
- **AI 缓存规则管理**：全局 AI 缓存规则集合（配合 BFE `mod_ai_cache` 模块，一期仅 Redis 精确匹配）；集合级全量读写（`GET`/`PUT /ai-cache-rules`，无 `/{id}` 端点），按数组顺序 first-match-wins，经 InnerAPI 导出 `ai_cache.data`。
- **AI 意图配置管理**：全局单例资源（配合 BFE `mod_ai_intent` 模块）；单行覆盖式全量读写（`GET`/`PUT /intent-config`，无 `/{id}` 端点、不保存历史版本），`questions` 空数组 = 停用意图分类软开关；经 InnerAPI 导出 `intent_questions.data`。
...
- **模型定价管理（`/model-prices`）**：导入 `model-list.yaml`、单条 CRUD（支持按 `(provider, model, mode)` 组合键查询/修改/删除，GET 组合键三参齐全返回单条记录）、分页查询、查询所有 Provider 名称列表（`GET /model-prices/actions/get-providers`）；`provider` 字段仅作为价格归集标识，不强制引用 `/providers` 中已存在的 Provider，为 RMB 配额与 BFE 成本核算提供统一模型定价数据源。
...
- **操作日志管理（`/operation-logs`）**：记录控制面配置变更操作，覆盖 cluster、route、domain、certificate、quota_plan、rate_limit_policy、ai_cache_rule、intent_config、model_price、user、token、entity、api_key、provider 等域；支持按操作人、资源类型、动作、时间范围分页查询；对 api-key token、密码、私钥、证书等敏感字段统一脱敏；采用异步批量写入降低主请求延迟，失败操作同样记录。

- **配置导出（InnerAPI）**：向 BFE/Conf Agent 导出 TLS/Server/路由、GSLB、证书、附加文件、API-Key、请求体处理、限流策略、AI 缓存规则（`/configs/ai-cache-rule`，产物 `ai_cache.data`）、AI 意图配置（`/configs/mod-ai-intent`，产物 `intent_questions.data`）、AI 路由等配置；`server_data_conf` 中 `ClusterConf.Config.<cluster_name>.AIConf` 新增 `Provider`、`ModelTable`、`MatchPrefix`、`StripPrefix` 与 `ModelProtocols`；EPP 模式 cluster 导出 `GslbBasic.BalanceMode=EPP` 与有序 `EPPAddr [主, 备]`（无有效分配时降级为该 cluster `BalanceMode=WRR` + error 日志，不阻塞整份下发）；向 EPP 实例导出 `epp_data`（`/configs/epp_data/config`）。
...
│ ├── icluster_conf/ # 集群/子集群/实例池；Cluster 只保留转发策略，后端能力通过 provider 引用获取
│ ├── iprovider/ # Provider 业务模型：CRUD、模型发现、被 cluster 引用检查
│ ├── imodel_price/ # ModelPrice 模型定价管理：`model-list.yaml` 导入、单条 CRUD、分页查询、按 provider 查询
│ ├── iroute_conf/ # 路由规则/域名
│ ├── iai_route/ # AI 路由规则
│ ├── iprotocol/ # TLS 证书
│ ├── iversion_control/ # 配置版本控制
│ ├── imods/ # 模块配置导出（mod-api-key、mod-body-process 等）
│ ├── ioperlog/ # 操作日志：异步缓冲、批量写入、脱敏、查询
│ ├── quota/ # QuotaPlan / BalanceSync / QuotaResetScheduler（含分布式锁）
│ ├── rate_limit_policy/ # RateLimitPolicy 业务模型
│ ├── ai_cache/ # AICacheRule 业务模型（AI 缓存规则集合，配合 BFE mod_ai_cache）
│ ├── iintent_config/ # IntentConfig 业务模型（全局单例，配合 BFE mod_ai_intent）
│ ├── quotacache/ # API-Key/Entity 实时配额缓存抽象（含 Redis 分布式锁实现）
│ ├── shared/ # 跨包共享类型与模型
...
| `model/*` | 业务模型、Manager、Storager 接口、Param/Filter；`model/quotacache` 封装实时配额缓存 |
...
`ai-gateway-api` 当前代码实现中共使用 34 张关系型数据表，按业务模块划分如下：
...
| `api_key_id_seq` | API-Key ID 序号分配表，按 `product_name` 原子分配 `api-key-{seq}` 序号，分配即消耗、删除后不回退 |
| `entities` | Entity 实体表，支持层级结构（`parent_id`）与模型白/黑名单 |
| `entity_id_seq` | Entity ID 序号分配表，原子分配 `entity-{seq}` 序号，分配即消耗、删除后旧 ID 不复用 |
| `entity_types` | Entity 类型定义表，定义类型名、层级级别 |
| `quota_plans` | 配额计划表，记录是否无限配额、配额总量、重置周期、上次重置时间等 |
| `rate_limit_policies` | 限流策略表，记录 TPM/RPM 配置与最大并发数 |
| `ai_cache_rules` | AI 缓存规则表，全局有序规则集合（first-match-wins），BFE `mod_ai_cache` 数据源；`id` 仅作内部排序不暴露 API，无 `enabled` 列 |
| `intent_config` | AI 意图配置表，单行覆盖式存储（固定 id=1 upsert，不保存历史版本），BFE `mod_ai_intent` 的 `intent_questions.data` 数据源；无 `enabled` 列，`version` 为下发链路内部字段（不在 OpenAPI 暴露） |
...
| `storage/rdb/ai_cache` | `model/ai_cache` | `ai_cache_rules` |
...
| `model/ioperlog` | 操作日志管理：定义 `OperationLogEntry`/`OperationLogFilter`，提供 `OperationLogManager` 异步批量写入、脱敏工具函数与查询能力 |
| `model/quota` | QuotaPlan、BalanceSync、QuotaResetScheduler；`QuotaPlan` 支持 `unit=RMB`，`quota` 扩展为 `float64` / `DECIMAL(18,8)`；`last_reset_at` 保存在 `quota_plans` 表；周期重置通过 Redis 分布式锁互斥，并采用原子 `SET` + `last_reset_at` 条件更新实现幂等；`quota_balances` 表已删除，OpenAPI 余额直接读 Redis |
| `model/rate_limit_policy` | RateLimitPolicy 业务逻辑、层级收集与导出 |
| `model/ai_cache` | AICacheRule 业务逻辑：AI 缓存规则集合全量读写（事务重建）、`ai_cache.data` 导出生成（topic `mod_ai_cache`）、PUT 审计 |
| `model/iintent_config` | IntentConfig 业务逻辑：AI 意图配置单例全量读写（单行覆盖、无历史版本）、意图配置整体校验（同 BFE 口径）、`intent_questions.data` 导出生成（topic `intent_config`）、PUT 审计 |
| `model/quotacache` | API-Key / Entity 实时配额缓存抽象，封装 Redis `GetInt64` / `IncrBy` / Lua `SET` 操作，并提供基于 Redis Lua 的 `DistributedLock` 实现，供 `model/quota`、`model/api_key`、`model/entity` 调用 |
...
| `provider` | `/providers` | Provider 管理 |
...
| `/configs/ai-cache-rule` | 导出 AI 缓存规则配置（BFE `mod_ai_cache` 数据源，产物 `ai_cache.data`） |
| `/configs/mod-ai-intent` | 导出 AI 意图配置（BFE `mod_ai_intent` 数据源，产物 `intent_questions.data`） |
...
1. **事务与基础存储**：`TxnStoragerSingleton`、`VersionControlStoragerSingleton`、`RouteRuleStoragerSingleton`、各类基础/集群/认证存储，以及 `OperationLogStorager`。
2. **操作日志 Manager**：创建 `OperationLogManager`，设置请求上下文提取器（注入 `log_id`、请求路径、客户端 IP、User-Agent 等），并作为全局单例供后续各业务 Manager 注入。
3. **基础 Manager**：`ExtraFileManager`、`VersionControlManager`、`BFEClusterManager`、`CertificateManager`、`ProductManager`、`AIRouteRuleManager`、`RouteRuleManager` 等。
4. **集群相关 Manager**：`ClusterManager`、`SubClusterManager`、`DomainManager`、`PoolManager`。
5. **认证授权 Manager**：`AuthenticateManager`、`AuthorizeManager`。
6. **Entity / RateLimit / Quota 相关存储与 Manager**：在 `model/entity`、`model/rate_limit_policy`、`model/ai_cache`、`model/iintent_config`、`model/quota` 中分别初始化 EntityType、Entity、RateLimitPolicy、AICacheRule、IntentConfig、QuotaPlan、RouteRules 及其 Manager。
7. **模块导出 Manager**：`APIKeyRuleManager`、`ModBodyProcessManager`、`AIRouteExporter`。
8. **API-Key Manager**：注入 Quota/RateLimit/Entity 适配器，实现 `model/api_key` 与配额/限流体系的打通。
9. **周期重置**：`BalanceSyncManager`、`QuotaResetScheduler`，并启动调度器；`QuotaResetScheduler` 注入 Redis 分布式锁，保证多实例下同一时刻只有一个实例执行重置；`BalanceSyncManager` 只执行 `ResetExpiredBalances`，不再执行 `SyncAllBalances`。
10. **Global 路由表默认记录**：调用 `RouteRulesManager.EnsureGlobalRouteRules` 确保 `route_rules` 表中存在 `type=global, owner=global` 的默认记录；若已存在则跳过。

### 8.3 适配器模式

API-Key、Entity、RateLimitPolicy 已拆分为独立的 `model/api_key`、`model/entity`、`model/rate_limit_policy` 包，而 QuotaPlan 仍保留在 `model/quota`。代码中通过**适配器**（如 `quota.NewQuotaPlanStoragerAdapter`、`quota.NewEntityStoragerAdapter`、`quota.NewRateLimitPolicyStoragerAdapter`）将这些包中的 Storager 接口转换为 `model/shared` 中定义的通用接口，供 `APIKeyManager` / `EntityManager` 统一调用，实现跨包复用。其中 `quota.NewEntityStoragerAdapter` 接收 `entity.EntityStorager`，`quota.NewRateLimitPolicyStoragerAdapter` 接收 `rate_limit_policy.RateLimitPolicyStorager`。
