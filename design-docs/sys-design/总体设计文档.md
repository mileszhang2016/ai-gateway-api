...
- **API-Key 管理**：创建、查询、更新、删除 API-Key；支持配额计划、限流策略和路由规则；额外提供 `/api-keys/actions/generate` 用于生成 Key。
- **Entity 管理**：创建、查询、更新、删除 Entity，支持层级结构、配额/限流和路由规则。
- **Entity-Type 管理**：创建、查询、更新、删除 Entity 类型定义。
- **配额计划管理**：查询和重置配额余额。
- **限流策略管理**：TPM、RPM、并发限制配置。
- **路由规则管理**：API-Key/Entity 级路由规则、Global 路由表、路由表列表查询。
- **AI 网关实例池管理（`/alb-pool`）**：获取和更新默认 AI 网关实例池。
- **认证授权管理（`/auth`）**：用户、Session Key、Token 及产品线授权。
- **证书管理（`/certificates`）**：证书创建、列表、设置默认证书、删除。
- **集群管理（`/clusters`）**：集群一键创建、查询、更新、删除、就绪检查及模型管理；额外提供 `/model-providers`、`/models`、`/global-models`。
...
| `product_cluster` | `/clusters`、`/model-providers`、`/models` | 集群与模型管理 |
| `models` | `/global-models` | 全局模型 |
| `certificate` | `/certificates` | 证书管理 |
| `bfe_pool` | `/alb-pool` | 默认 AI 实例池 |
| `auth` | `/auth`、`/meta` | 用户、Session Key、Token 与产品线授权 |
| `general` | `/general/actions/exec-api` | 通用接口 |
...
main.go
  ...
  ├─ rdb.Init()                      # 依赖注入：初始化所有 Manager/Storager
  │
  └─ serverStartUp()
...
| 主题 | 文档路径 |
|------|----------|
| 接口定义（OpenAPI） | `ai-gateway-api/design-docs/api-define/OpenAPI接口定义.md` |
| 接口定义（InnerAPI） | `ai-gateway-api/design-docs/api-define/InnerAPI接口定义.md` |
