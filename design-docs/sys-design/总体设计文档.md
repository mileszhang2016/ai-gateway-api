...
| `model/quota` | EntityType、Entity、QuotaPlan、QuotaBalance、RateLimitPolicy、BalanceSync、QuotaResetScheduler；`QuotaPlan` 支持 `unit=RMB`，`quota` / `used` / `remaining` 扩展为 `float64` / `DECIMAL(18,8)` |
| `model/quotacache` | API-Key / Entity 实时配额缓存抽象，封装 Redis `GetInt64` / `IncrBy` 操作，供 `model/quota` 与 `model/icluster_conf` 调用 |
