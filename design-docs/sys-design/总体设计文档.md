...
| `storage/rdb/cluster_conf` | `model/icluster_conf`、`model/api_key` | `clusters`、`sub_clusters`、`pools`、`api_keys`、`lb_matrices` |
| `storage/rdb/protocol` | `model/iprotocol` | `certificates` |
| `storage/rdb/quota` | `model/entity`、`model/rate_limit_policy`、`model/quota`、`model/shared` | `entity_types`、`entities`、`quota_plans`、`quota_balances`、`rate_limit_policies`、`route_rules` |
...
| `model/icluster_conf` | Cluster、SubCluster、Pool 业务逻辑；`LLMConfig` 新增 `provider`、`match_prefix`、`strip_prefix`，导出 `AIConf` 时填充 `Provider`、`ModelTable`、`MatchPrefix` 与 `StripPrefix` |
