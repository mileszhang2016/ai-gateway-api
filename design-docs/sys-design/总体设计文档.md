...
- **Provider 管理（`/providers`）**：创建、查询、更新、删除 Provider；支持 `model_endpoint`、`models`、`keys`、`instance_pool`、`model_protocols` 等字段，集中管理下游模型提供方的接入能力；提供 `/providers/{provider_name}/discover-models` 触发模型发现，自动回填 `models`。
- **集群管理（`/clusters`）**：集群一键创建、查询、更新、删除；`llm_config` 只保留转发策略（含 `models`、`model_mappings`、`keys` Key 引用与权重、`key_policy`、`provider`、`match_prefix`/`strip_prefix` 等），后端实例池、Key 明文、模型端点等能力通过 `llm_config.provider` 引用 Provider 获取；集群就绪状态、子集群、调度矩阵等内部概念不再对外暴露；`llm_config.provider` 用于 InnerAPI 自动填充 `AIConf.ModelTable`。
- **模型定价管理（`/model-prices`）**：导入 `model-list.yaml`、单条 CRUD、分页查询；`provider` 字段必须引用 `/providers` 中已存在的 Provider，为 RMB 配额与 BFE 成本核算提供统一模型定价数据源。
...
│ ├── iprovider/ # Provider 业务模型：CRUD、模型发现、被 cluster/model-prices 引用检查
...
│ ├── quota/ # QuotaPlan / QuotaBalance / BalanceSync / QuotaResetScheduler
...
`ai-gateway-api` 当前代码实现中共使用 26 张关系型数据表，按业务模块划分如下：
...
| `storage/rdb/quota` | `model/quota` | `quota_plans`、`quota_balances` |
...
| `model/icluster_conf` | Cluster、SubCluster、Pool 业务逻辑；`LLMConfig` 只保留转发策略，通过 `provider` 引用 Provider 获取后端能力；导出 `AIConf` 时按 name join Provider 的 key 明文与 Cluster 的 key 权重，填充 `Provider`、`ModelTable`、`MatchPrefix` 与 `StripPrefix` |
| `model/iprovider` | Provider 业务逻辑：CRUD、模型发现、被 cluster/model-prices 引用检查 |
| `model/imodel_price` | `ModelPrice` 模型定价管理：`model-list.yaml` 导入、单条 CRUD、分页查询、按 provider 查询；`provider` 须引用已存在的 Provider |
...
| `model/quota` | QuotaPlan、QuotaBalance、BalanceSync、QuotaResetScheduler；`QuotaPlan` 支持 `unit=RMB`，`quota` / `used` / `remaining` 扩展为 `float64` / `DECIMAL(18,8)` |
| `model/rate_limit_policy` | RateLimitPolicy 业务逻辑、层级收集与导出 |
| `model/quotacache` | API-Key / Entity 实时配额缓存抽象，封装 Redis `GetInt64` / `IncrBy` 操作，供 `model/quota`、`model/api_key`、`model/entity` 调用 |
...
5. **Entity / RateLimit / Quota 相关存储与 Manager**：在 `model/entity`、`model/rate_limit_policy`、`model/quota` 中分别初始化 EntityType、Entity、RateLimitPolicy、QuotaPlan、QuotaBalance、RouteRules 及其 Manager。
6. **模块导出 Manager**：`APIKeyRuleManager`、`ModBodyProcessManager`、`AIRouteExporter`。
7. **API-Key Manager**：注入 Quota/RateLimit/Entity/Balance 适配器，实现 `model/api_key` 与配额/限流体系的打通。
...
API-Key、Entity、RateLimitPolicy 已拆分为独立的 `model/api_key`、`model/entity`、`model/rate_limit_policy` 包，而 QuotaPlan / QuotaBalance 仍保留在 `model/quota`。代码中通过**适配器**（如 `quota.NewQuotaPlanStoragerAdapter`、`quota.NewEntityStoragerAdapter`、`quota.NewRateLimitPolicyStoragerAdapter`、`quota.NewQuotaBalanceStoragerAdapter`）将这些包中的 Storager 接口转换为 `model/shared` 中定义的通用接口，供 `APIKeyManager` / `EntityManager` 统一调用，实现跨包复用。其中 `quota.NewEntityStoragerAdapter` 接收 `entity.EntityStorager`，`quota.NewRateLimitPolicyStoragerAdapter` 接收 `rate_limit_policy.RateLimitPolicyStorager`。
