...
Data为数组，元素字段同2.1数据模型，但`quota_plan`中的`balance`字段不返回（仅查询单个API-Key或调用独立quota-plan查询接口时返回）。
...
| is_admin | bool | 是否为系统管理员 | Y | `true`：System 权限；`false`：Product 权限 |
