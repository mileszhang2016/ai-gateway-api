...
| RT-1-010 | 空列表返回 | 边界值 | 无任何路由表时返回空列表（系统初始化后默认存在 global 路由表，需全新数据库验证） |
...
#### 6.4.12 RT-1-012：启动后默认 global 路由表存在（边界值）

##### 设计思路

验证系统初始化后，`/route-tables` 接口能直接查询到默认 global 路由表。
...
1. 发送 GET 请求，`type=global`。
2. 验证返回列表长度为 1，且元素为默认 global 路由表（`type=global`、`owner=global`、`enabled=false`）。
...
|------|--------|---------|
| list | 长度为 1 | Len=1 |
| list[0].type | "global" | Equals |
| list[0].owner | "global" | Equals |
| list[0].enabled | false | Equals |
