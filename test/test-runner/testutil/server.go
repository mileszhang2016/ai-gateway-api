...
func StartServer() (*ServerManager, error) {
	...
	// 1. 获取 test-runner 目录和项目根目录
	testRoot, err := findTestRunnerRoot()
	if err != nil {
		return nil, fmt.Errorf("find test-runner root: %w", err)
	}
...
// findTestRunnerRoot 查找 test-runner 目录
func findTestRunnerRoot() (string, error) {
...
func findProjectRoot() (string, error) {
	...
	for {
		...
		if _, err := os.Stat(goModPath); err == nil {
			...
			if err == nil && strings.Contains(string(content), "module github.com/yf-networks/ai-gateway-api") && !strings.Contains(string(content), "test-runner") {
				return dir, nil
			}
		}
